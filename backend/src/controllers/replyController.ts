...
const ReplyControl = {
    add: async(req:Request,res:Response) =>{
        try{
            ...
            const post= await Post.findById(postId).populate<{ author: { email: string; username: string; id: string } }>("author", "email username")
    ...
    getReplies: async(req:Request, res:Response)=>{
        try{
            const {postId} = req.params
            const replies = await Reply.find({post:postId}).populate("author","email username")
            .sort({createdAt:-1})
